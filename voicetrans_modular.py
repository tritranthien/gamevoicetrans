...
class TranslatorEngine:
    def __init__(self, settings, ui):
        ...
        self.ui.log(f"✅ Whisper model loaded on {device.upper()}", 'info')
        
        # Translator
        self.translator = GoogleTranslator(source='zh-CN', target='vi')
        
        # TTS Engine (using our new module!)
                        ...
                        if silence_counter > pause_frames:
                            if len(buffer) > min_audio_frames:
                                ...
                                if not self.audio_queue.full():
                                    ...
                                    self.ui.log(f"📤 Audio sent ({len(buffer)} frames)", 'info')
    ...
    def speech_to_text_thread(self):
        """STT thread with gender detection"""
        while self.is_running:
            try:
...
# UI Class remains mostly the same, just update TTS selection
class AudioTranslatorUI:
    ...
    def create_ui(self):
        ...
        self.gpu_label.grid(row=8, column=0, columnspan=2, pady=5)
    ...
    def create_sliders(self, parent):
        ...
        # Silence Threshold
        ttk.Label(parent, text="🔇 Silence Threshold:").grid(row=row, column=0, sticky=tk.W, pady=5)
        ...
        # Pause Time
        ttk.Label(parent, text="⏸️ Pause Time (s):").grid(row=row, column=0, sticky=tk.W, pady=5)
    ...
    def create_log_area(self):
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, bg='#1e1e1e', fg=Config.COLORS['success'],
                                                 font=('Consolas', 9), height=15, wrap=tk.WORD)
    ...
    def start_translation(self):
        ...
        settings = {
            ...
            'tts_engine': self.tts_engine_var.get(),  # NEW!
...
def main():
    print("=" * 50)
    print("Real-time Audio Translator - Modular Version")
    print("=" * 50)
